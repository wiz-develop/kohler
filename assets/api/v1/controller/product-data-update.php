    ...
    if (($handle = fopen($csv_file, "r")) !== FALSE) {
        ...
        while (($data = fgetcsv($handle, 10000, ",")) !== FALSE) {
            ...
            if ($id != 'ID') {
                ...
                merge_document_list_from_row($products[$index]['field']['document_list'], $data);
...
function normalize_document_file_value($value)
{
    ...
    return $attachment_id ? (string) $attachment_id : $value;
}

function merge_document_list_from_row(&$document_list, $data)
{
    for ($i = 0; $i < 7; $i++) {
        $offset = 44 + ($i * 3);
        $document_name = (isset($data[$offset])) ? trim((string) $data[$offset]) : '';
        $document_url = normalize_document_file_value((isset($data[$offset + 1])) ? $data[$offset + 1] : '');
        $document_link_blank = (isset($data[$offset + 2])) ? trim((string) $data[$offset + 2]) : '';

        if (!isset($document_list[$i])) {
            $document_list[$i] = array(
                'document_name'       => '',
                'document_url'        => '',
                'document_link_blank' => '',
            );
        ...
        if ($document_name !== '') {
            $document_list[$i]['document_name'] = $document_name;
        }

        if ($document_url !== '') {
            $document_list[$i]['document_url'] = $document_url;
        }

        if ($document_link_blank !== '') {
            $document_list[$i]['document_link_blank'] = $document_link_blank;
        }
    }
}
