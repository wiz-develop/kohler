									...
									<ul class="product-item_detail document_list ps-3 mb-0">
										<?php 
											foreach ($valid_documents as $document_lists) :
												...
												$document_file_name = basename(rawurldecode($document_path));
												...
												$download_file_name = $document_file_name;
												if ($document_download) {
													$download_base = strtoupper(preg_replace('/[^A-Za-z0-9]+/', '-', get_post_field('post_name', get_the_ID())));
													$download_base = trim($download_base, '-');
													$document_label = strtolower($document_name);
													if (strpos($document_label, '2d') !== false) {
														$download_suffix = '2d';
													} elseif (strpos($document_label, '3d') !== false) {
														$download_suffix = '3d';
													} else {
														$download_suffix = $document_ext ?: 'file';
													}
													$download_file_name = $download_base . '_' . $download_suffix . ($document_ext ? '.' . $document_ext : '');
												}
										?>
										<li class="document_list__item">
											<a href="<?php echo esc_url($document_url); ?>" <?php if ((int) $document_link_blank === 1) echo 'target="_blank" rel="noopener"'; ?> <?php if ($document_download) echo 'download="' . esc_attr($download_file_name) . '"'; ?>>
